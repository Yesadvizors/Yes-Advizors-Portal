...
Authenticated UAT — simulate a transient team-lookup failure and confirm: (a) sign-in shows the retryable message, not "account not active"; (b) session-restore shows the Retry screen, no silent logout; (c) genuine inactive account still shows the accurate message; (d) password-reset errors are business-safe. Checklist: `docs/YAV2_AUTH_SESSION_RESILIENCE_UAT_CHECKLIST.md`.
