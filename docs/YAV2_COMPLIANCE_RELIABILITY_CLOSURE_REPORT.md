...
| New tests | 1 | `tests/complianceReliabilityClosure.test.js` (**25** tests) |
...
- **Tests:** `node --test` → **399 → 424 pass / 0 fail** (+25).
...
The build (all modules transformed) plus the 424 pure/static tests are the runtime verification performed.
