...
- **WM-1 (HIGH) — false-empty-on-outage closed.** `load()` discarded the `error` on all three reads (`tasks`, `follow_ups`, `team`); a failed load rendered as the empty "No tasks match your filters." — indistinguishable from "no tasks." `load()` now checks each query's `error`, logs it (console only), and the list renders a **retryable error state distinct from empty** (mirrors the proven `loadError` pattern in `Clients.jsx`). This is the same false-empty class PR #51 fixed for Compliance; `Tasks.jsx` had been missed.
...
- **CL-1 — directors fetch hardened.** The `client_directors` fetch discarded its `error` and had no race guard (a late response for a previously-viewed client could overwrite the current one). Now: error surfaced to console (no silent discard); a **stale-response race guard** (`ignore` flag in the effect cleanup) drops a response after the viewed client changed; the legacy `c.directors` fallback is preserved (populate only when rows exist).
...
Added `CLIENT_LIFECYCLE_STATUSES`, `clientStatusLabel`, `isFollowUpOverdue`, `isFollowUpToday` (pure, tested). Reuse-first — no parallel logic introduced.

## 4. Tests added
`tests/clientLifecycleWorkManagementClosure.test.js` — **10 tests** (CLW-1..10): pure-logic (status label never defaults Active; known/legacy verbatim; follow-up due-today-not-overdue; blank/past/future) + static source guards (Tasks load-error wiring; exact assignee; shared follow-up predicates + no non-owner alert; directors race/error guard; no `null→Active`; consistent label + filter; single Escape handler). Convention OD-5 — no jsdom/RTL, no invented DB shape; static/logic only.

## 5. Test / build / runtime results
- **Full suite: 487 pass / 0 fail** (477 baseline + 10 CLW). **Build: exit 0.**
...
Interactive authenticated smoke (Admin + non-Admin) needs a governed test-account credential + a git-ignored `.env.local` (absent in this worktree). Repository logic is covered by the 487 pure/static tests, the clean build (all modules transformed), and the non-auth HTTP-200 shell check; interactive login is recorded as a separate manual dependency (same standing limitation as the prior closures).
