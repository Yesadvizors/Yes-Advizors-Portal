...
**No secret values requested/exposed.** Duplicate/stale projects: the second project is a duplicate building the same branch (app-source equivalent). **V1 reference:** none observed in provenance metadata; env-value V1/V2 confirmation is `UNVERIFIED — MANUAL PJ EVIDENCE REQUIRED`. **PJ evidence needed:** for each variable/project — name · scope · present/missing · V2-target confirmed (screenshot/redacted export), never secret values.

## A4 — Supabase V2 discovery package
Delivered as `supabase/verification/YAV2_Package_A_V2_Live_State_Discovery_Readonly.sql` — **PJ executes on V2 (`ogjrwemjefvccpyjwxuo`) only; never V1.** Read-only (SELECT-only; session forced `READ ONLY`; no DDL/DML/mutating calls/role/grant changes; no secret/hash/token selection). Sections: 0 environment identity (+abort-if-not-V2) · 1 schemas · 2 tables/columns · 3 constraints (PK/FK/UNIQUE/CHECK) · 4 indexes · 5 enums · 6 views/matviews · 7 functions (security mode/owner/`search_path`) · 8 triggers · 9 migration reconciliation (+expected-object existence for `0001–0022`, `0012`/`0019` notes, `0014`/`0021`/`0022`) · 10 RLS/FORCE-RLS/policies/grants/function-execute · 11 Auth users + team + Auth↔team integrity (counts/status only) · 12 module data counts · 13 storage buckets/objects/policies · 14 Edge-function name hints · 15 audit tables/functions. **PJ pastes the full sectioned output back for the Package A/B registers.**
...
All **Claude-side read-only discovery is complete** (A1 baseline/preservation; A2 governing source inventory; A3 Vercel provenance; A4 discovery SQL authored; A5/A7 evidence templates; A6 Edge/storage register; A8 approval-to-live register with every item classified — no `UNKNOWN`; A9 indicators). **Live verification cannot proceed within standing guardrails** (Claude never executes SQL; connected Supabase MCP reaches only prohibited V1; no approved test-account credentials in-session; Vercel env values not exposed). **PJ evidence required to advance to Package B:** (1) run the A4 discovery SQL on V2 and return the full output; (2) supply Vercel env-var presence/scope/V2-target evidence (redacted, no secrets); (3) confirm the exact governing portal URL PJ uses; (4) provide approved test-account runtime evidence (A5); (5) provide n8n/WhatsApp export or screen evidence (A7). **No live alignment is claimed.**
