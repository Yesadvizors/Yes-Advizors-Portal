...
- **This first package is design + initial inventory only** (executable recovery design + initial source/history
  ...
  Edge/storage/n8n evidence are **Package A/B execution deliverables**. *(Design PR #24 merged `b2ceb30…`.)*
...
- **Package A PJ Evidence Execution Pack active (Issue #23 — PR #26):** documentation-only manual procedure
  ...
  runtime evidence, no user creation/reset), E5 (n8n/WhatsApp inventory, no execution/secret export). **Package A stays
  `PARTIAL`** until the evidence is returned and reviewed; **Package B not authorised.** Nothing executed or changed.
