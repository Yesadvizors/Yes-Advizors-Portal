...
-- Dependency closure (scope_map Phase 2). Byte-exact from V1 reference (REFERENCE ONLY),
-- re-extracted with correct UTF-8 (fixes Rev1.0 mojibake, C-G2B-4).
-- Includes the full audit-validation function chain required by _write_read_audit /
-- get_sensitive_audit_logs (C-G2B-1): audit_contains_secret, audit_is_uuid,
-- audit_field_format_ok, audit_validate_event, _record_audit_failure, _write_read_audit.
-- Order: helpers before callers. All plpgsql resolve called fns at runtime; SQL fns' tables pre-exist.

-- audit_contains_secret (ref L1099-1118)
CREATE FUNCTION public.audit_contains_secret(p_text text) RETURNS boolean
...
-- audit_is_uuid (ref L1205-1215) — called by audit_field_format_ok
CREATE FUNCTION public.audit_is_uuid(p_text text) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE SECURITY DEFINER
    SET search_path TO 'pg_catalog', 'public', 'pg_temp'
    AS $$
DECLARE v uuid;
...
-- audit_field_format_ok (ref L1125-1198) — calls audit_is_uuid
CREATE FUNCTION public.audit_field_format_ok(p_field text, p_value jsonb) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE SECURITY DEFINER
    SET search_path TO 'pg_catalog', 'public', 'pg_temp'
    AS $_$
DECLARE
...
-- audit_validate_event (ref L1222-1274) — calls audit_field_format_ok, audit_contains_secret
CREATE FUNCTION public.audit_validate_event(p_event_name text, p_initiated_by_type text, p_action text, p_resource_type text, p_client_uuid uuid, p_target_user_id uuid, p_metadata jsonb) RETURNS text
...
-- _record_audit_failure (ref L980-991)
CREATE FUNCTION public._record_audit_failure(p_event_name text, p_failure_reason text, p_field_names_only text[], p_sqlstate text) RETURNS void
...
-- _write_read_audit (ref L998-1039) — called by get_sensitive_audit_logs; calls the above + get_app_role_for_user (0008)
CREATE FUNCTION public._write_read_audit(p_event_name text, p_client_uuid uuid, p_metadata jsonb) RETURNS uuid
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'pg_catalog', 'public', 'pg_temp'
    AS $$
DECLARE
...
-- calc_gst_due_date (ref L1316-1340) — called by generate_client_compliance
CREATE FUNCTION public.calc_gst_due_date(p_return_type text, p_month public.month_enum, p_fy_year integer) RETURNS date
...
DECLARE
  ...
  v_due_year  INT;
BEGIN
  v_cal_month := gst_month_to_num(p_month);
  -- Calendar year: Apr-Dec = fy_year, Jan-Mar = fy_year+1
  v_cal_year := CASE WHEN v_cal_month >= 4 THEN p_fy_year ELSE p_fy_year + 1 END;

  -- Due date is next month: handle December → January year rollover
  v_due_month := CASE WHEN v_cal_month = 12 THEN 1 ELSE v_cal_month + 1 END;
  v_due_year  := CASE WHEN v_cal_month = 12 THEN v_cal_year + 1 ELSE v_cal_year END;

  RETURN CASE p_return_type
    WHEN 'GSTR-1'  THEN make_date(v_due_year, v_due_month, 11)
    WHEN 'GSTR-3B' THEN make_date(v_due_year, v_due_month, 20)
    ELSE NULL
  END;
END;
$$;

-- get_client_start_fy (ref L3676-3695) — called by generate_client_compliance
CREATE FUNCTION public.get_client_start_fy(p_incorporation_date date) RETURNS character varying
    LANGUAGE plpgsql
    SET search_path TO 'public', 'pg_temp'
    AS $$
DECLARE
  v_incorp_fy VARCHAR(10);
  v_base_fy   VARCHAR(10) := '2020-21';
  v_year INTEGER; v_month INTEGER;
BEGIN
  IF p_incorporation_date IS NULL THEN RETURN v_base_fy; END IF;
  v_year  := EXTRACT(YEAR FROM p_incorporation_date);
  v_month := EXTRACT(MONTH FROM p_incorporation_date);
  IF v_month >= 4 THEN
    v_incorp_fy := v_year::TEXT || '-' || LPAD(((v_year + 1) % 100)::TEXT, 2, '0');
  ELSE
    v_incorp_fy := (v_year - 1)::TEXT || '-' || LPAD((v_year % 100)::TEXT, 2, '0');
  END IF;
  IF v_incorp_fy > v_base_fy THEN RETURN v_incorp_fy; ELSE RETURN v_base_fy; END IF;
END;
$$;

-- Dependency-KEEP tables (byte-exact) — reclassified DROP->DEPENDENCY-KEEP (C-G2B-3)
-- ct_team_members (ref L7399-7413) — read by get_my_role/get_my_team_id
CREATE TABLE public.ct_team_members (
    ...
    role public.user_role_enum DEFAULT 'Team Member'::public.user_role_enum NOT NULL,
...
ALTER TABLE ONLY public.ct_team_members ADD CONSTRAINT ct_team_members_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.ct_team_members ADD CONSTRAINT ct_team_members_email_key UNIQUE (email); -- ref L9240

-- llp_tracker (ref L7908-7940) — INSERT target of generate_client_compliance; UNIONed by 2 views
CREATE TABLE public.llp_tracker (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    client_id uuid NOT NULL,
    fy_id uuid,
    fy_label character varying(10) NOT NULL,
    llpin character varying(20),
    form_name character varying(100) NOT NULL,
    filing_type public.roc_filing_type_enum NOT NULL,
    event_date date,
    ...
    partner_approved_date timestamp with time zone,
    filed_by uuid,
...
ALTER TABLE ONLY public.llp_tracker ADD CONSTRAINT llp_tracker_pkey PRIMARY KEY (id);

-- payroll_tracker (ref L7996-8039) — UNIONed by 2 views
CREATE TABLE public.payroll_tracker (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    client_id uuid NOT NULL,
    fy_id uuid,
    fy_label character varying(10) NOT NULL,
    month public.month_enum NOT NULL,
    period_label character varying(30),
    employee_data_received boolean DEFAULT false,
    attendance_received boolean DEFAULT false,
    salary_processed boolean DEFAULT false,
    payslip_generated boolean DEFAULT false,
    tds_on_salary_computed boolean DEFAULT false,
    pf_applicable boolean DEFAULT false,
    pf_challan_generated boolean DEFAULT false,
    pf_payment_done boolean DEFAULT false,
    pf_payment_date date,
    pf_amount numeric(12,2),
    ecr_filed boolean DEFAULT false,
    esi_applicable boolean DEFAULT false,
    esi_challan_generated boolean DEFAULT false,
    esi_payment_done boolean DEFAULT false,
    esi_payment_date date,
    esi_amount numeric(12,2),
    professional_tax_applicable boolean DEFAULT false,
    professional_tax_paid boolean DEFAULT false,
    lwf_applicable boolean DEFAULT false,
    lwf_paid boolean DEFAULT false,
    standard_due_date date,
    extended_due_date date,
    individual_due_date date,
    workflow_stage public.workflow_stage_enum DEFAULT 'Assigned'::public.workflow_stage_enum,
...
ALTER TABLE ONLY public.payroll_tracker ADD CONSTRAINT payroll_tracker_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.payroll_tracker ADD CONSTRAINT payroll_tracker_client_id_fy_label_month_key UNIQUE (client_id, fy_label, month); -- ref L9472

-- tds_client_config (ref L8152-8162) — INSERT target of generate_client_compliance
CREATE TABLE public.tds_client_config (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    client_id uuid NOT NULL,
    tan character varying(10),
    form_24q boolean DEFAULT false,
    form_26q boolean DEFAULT false,
    form_27q boolean DEFAULT false,
    form_27eq boolean DEFAULT false,
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now()
);
ALTER TABLE ONLY public.tds_client_config ADD CONSTRAINT tds_client_config_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.tds_client_config ADD CONSTRAINT tds_client_config_client_id_key UNIQUE (client_id); -- ref L9520

-- trust_ngo_tracker (ref L8257-8286) — UNIONed by 2 views
CREATE TABLE public.trust_ngo_tracker (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    client_id uuid NOT NULL,
    fy_id uuid,
    fy_label character varying(10) NOT NULL,
    compliance_name character varying(200) NOT NULL,
    compliance_code character varying(50),
...
ALTER TABLE ONLY public.trust_ngo_tracker ADD CONSTRAINT trust_ngo_tracker_pkey PRIMARY KEY (id);

-- UNIQUE constraints backing RPC ON CONFLICT targets + V1 parity (verbatim from reference ALTER section)
ALTER TABLE ONLY public.accounting_tracker ADD CONSTRAINT accounting_tracker_client_id_fy_label_month_key UNIQUE (client_id, fy_label, month); -- ref L9088
ALTER TABLE ONLY public.audit_tracker ADD CONSTRAINT audit_tracker_client_id_audit_type_fy_label_key UNIQUE (client_id, audit_type, fy_label); -- ref L9136
ALTER TABLE ONLY public.client_financials ADD CONSTRAINT client_financials_client_id_fy_label_key UNIQUE (client_id, fy_label); -- ref L9176
ALTER TABLE ONLY public.financials_tracker ADD CONSTRAINT financials_tracker_client_id_fy_label_doc_type_key UNIQUE (client_id, fy_label, doc_type); -- ref L9392
ALTER TABLE ONLY public.gst_tracker ADD CONSTRAINT gst_tracker_client_id_gstin_return_type_fy_label_period_key UNIQUE (client_id, gstin, return_type, fy_label, period); -- ref L9424
ALTER TABLE ONLY public.income_tax_tracker ADD CONSTRAINT income_tax_tracker_client_id_fy_label_key UNIQUE (client_id, fy_label); -- ref L9440
ALTER TABLE ONLY public.tds_tracker ADD CONSTRAINT tds_tracker_client_id_form_type_quarter_fy_label_key UNIQUE (client_id, form_type, quarter, fy_label); -- ref L9536
ALTER TABLE ONLY public.team ADD CONSTRAINT team_email_key UNIQUE (email); -- ref L9568

-- RLS enable on the 5 dependency tables (refined policies in 0010 Section 5)
ALTER TABLE public.ct_team_members ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.llp_tracker ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.payroll_tracker ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.tds_client_config ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.trust_ngo_tracker ENABLE ROW LEVEL SECURITY;
