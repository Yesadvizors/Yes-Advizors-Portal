...
-- REAL UUID validation (finding #10): attempt a cast inside an exception block.
...
-- Explicit per-field validation: enums, real int/date casts, ranges.
...
DECLARE
  ...
  ts timestamptz;
BEGIN
  -- null handled by caller (required vs optional); here treat null as "format n/a"
  ...
  CASE p_field
    ...
    -- reason / value / setting codes: documented constrained code format
    ...
    -- INTEGERS (finding #12): JSON number, no fractional, non-negative, bounded
    ...
    -- completion correlation + outcome (findings v8 #1/#2)
    ...
    -- DATES (finding #12): real date cast catches 2026-99-99
    ...
    -- TIMESTAMPS (findings #2, #5): canonical UTC YYYY-MM-DDTHH:MI:SS.MSZ;
    -- validated by real timestamptz parsing (not regex). Field names match the
    -- reader: filter_date_start / filter_date_end.
    ...
    -- short descriptive strings (bounded; no enum)
    ...
    ELSE
      RETURN false;  -- unknown field => reject (no broad _code acceptance)
  END CASE;
END;
$$;
...
-- Contract enforcement. Returns NULL when valid, else a reason code.
...
BEGIN
  ...
  -- target-user requirement (finding #6)
  ...
  -- metadata object + size + empty rule
  ...
  -- required keys: present AND non-null AND (if string) non-blank AND format-valid (finding #1)
  ...
  -- all supplied keys: allowed, non-nested, format-valid, length+secret
  FOR k, v IN SELECT key, value FROM jsonb_each(p_metadata) LOOP
    ...
    -- optional explicitly-null: reject (finding #1 "reject explicitly supplied null for optional")
