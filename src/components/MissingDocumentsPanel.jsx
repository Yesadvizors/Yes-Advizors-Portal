...
// requirement with no CURRENT linked document, filterable, with Create Task + Manage actions.
export default function MissingDocumentsPanel({ clients = [], user, onManage }) {
  ...
  const fyOptions = useMemo(() => [...new Set(rows.map(r => r.fy_label).filter(Boolean))].sort().reverse(), [rows])
  const typeOptions = useMemo(() => [...new Set(rows.map(r => r.requirement_ref_type))].sort(), [rows])
  const filtered = useMemo(() => rows.filter(r =>
    (!fClient || r.client_id === fClient) && (!fFY || r.fy_label === fFY) && (!fType || r.requirement_ref_type === fType)
  ), [rows, fClient, fFY, fType])
      ...
      {loading ? <div className="dh-empty">Loading readiness…</div>
        ...
        : (
          <table className="dh-table">
            <thead><tr><th>Client</th><th>Requirement</th><th>Area</th><th>FY / Period</th><th>Due</th><th style={{ textAlign: 'right' }}>Action</th></tr></thead>
            <tbody>
              {filtered.slice(0, 300).map(r => (
                <tr key={`${r.requirement_ref_type}:${r.requirement_ref_id}`}>
                  ...
                  <td><span className="dh-badge" style={{ background: '#EEF2FF', color: '#3730A3' }}>{r.requirement_ref_type}</span></td>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <span style={{ display: 'inline-flex', gap: 6 }}>
                      <button className="dh-ibtn" title="Manage documents" style={{ width: 'auto', padding: '0 10px', fontSize: 11, fontWeight: 600 }}
                        onClick={() => onManage?.({ refType: r.requirement_ref_type, refId: r.requirement_ref_id, clientId: r.client_id, clientName: r.client_name, fyLabel: r.fy_label, docType: r.doc_type, requirementLabel: r.requirement_label, period: r.period })}>Manage</button>
                      {canUpload && (
