...
function ITTab({ clientId, fy }) {
  ...
  useEffect(() => { setLoad(true); supabase.from('income_tax_tracker').select('*,ct_team_members!income_tax_tracker_assigned_to_fkey(display_name)').eq('client_id',clientId).eq('fy_label',fy).then(({data})=>{setRows(data||[]);setLoad(false)}) },[clientId,fy])
  if(load) return <Spin />
  return <CTTable cols={['FY','AY','ITR Form','Due Date','Data','Computation','Filed','Ack No.','Status','Assigned']} rows={rows} empty={<Empty label="Income Tax"/>} render={r=>(<><TD bold>{r.fy_label}</TD><TD>{r.assessment_year}</TD><TD>{r.itr_form}</TD><TD>{fmt(eff(r))}</TD><TD><YN v={r.data_received}/></TD><TD><YN v={r.computation_prepared}/></TD><TD><YN v={r.return_filed}/></TD><TD>{r.acknowledgement_number}</TD><TD><SBadge status={r.status}/></TD><TD>{r.ct_team_members?.display_name}</TD></>)} />
}
function GSTTab({ clientId, fy }) {
  const [rows, setRows] = useState([]); const [load, setLoad] = useState(true); const [filter, setFilter] = useState('All')
  useEffect(() => { setLoad(true); supabase.from('gst_tracker').select('*,ct_team_members!gst_tracker_assigned_to_fkey(display_name)').eq('client_id',clientId).eq('fy_label',fy).order('period_month').order('return_type').then(({data})=>{setRows(data||[]);setLoad(false)}) },[clientId,fy])
  ...
  return (<>
    ...
    <CTTable cols={['Period','Return','Due Date','Data','Reconciled','Payment','Filed','ARN','Status','Assigned']} rows={filtered} empty={<Empty label="GST"/>} render={r=>(<><TD bold>{r.period}</TD><TD>{r.return_type}</TD><TD>{fmt(eff(r))}</TD><TD><YN v={r.sales_data_received&&r.purchase_data_received}/></TD><TD><YN v={r.reconciliation_completed}/></TD><TD><YN v={r.payment_done}/></TD><TD><YN v={r.return_filed}/></TD><TD>{r.arn}</TD><TD><SBadge status={r.status}/></TD><TD>{r.ct_team_members?.display_name}</TD></>)} />
  </>)
}
function TDSTab({ clientId, fy }) {
  const [rows, setRows] = useState([]); const [load, setLoad] = useState(true)
  useEffect(() => { setLoad(true); supabase.from('tds_tracker').select('*,ct_team_members!tds_tracker_assigned_to_fkey(display_name)').eq('client_id',clientId).eq('fy_label',fy).order('quarter').order('form_type').then(({data})=>{setRows(data||[]);setLoad(false)}) },[clientId,fy])
  if(load) return <Spin />
  return <CTTable cols={['Quarter','Form','TAN','Due Date','Challan','Prepared','Filed','PRN','Status','Assigned']} rows={rows} empty={<Empty label="TDS"/>} render={r=>(<><TD bold>{r.quarter}</TD><TD>{r.form_type}</TD><TD>{r.tan}</TD><TD>{fmt(eff(r))}</TD><TD><YN v={r.challan_received}/></TD><TD><YN v={r.return_prepared}/></TD><TD><YN v={r.return_filed}/></TD><TD>{r.token_number}</TD><TD><SBadge status={r.status}/></TD><TD>{r.ct_team_members?.display_name}</TD></>)} />
}
function ROCTab({ clientId, fy }) {
  const [rows, setRows] = useState([]); const [load, setLoad] = useState(true); const [filter, setFilter] = useState('All')
  useEffect(() => { setLoad(true); supabase.from('roc_tracker').select('*,ct_team_members!roc_tracker_assigned_to_fkey(display_name)').eq('client_id',clientId).eq('fy_label',fy).order('filing_type').order('form_name').then(({data})=>{setRows(data||[]);setLoad(false)}) },[clientId,fy])
  ...
  return (<>
    ...
    <CTTable cols={['Form','Type','Due Date','Docs','Prepared','Filed','SRN','Status','Assigned']} rows={filtered} empty={<Empty label="ROC"/>} render={r=>(<><TD bold>{r.form_name}</TD><TD><span style={{fontSize:10,padding:'2px 8px',borderRadius:99,background:r.filing_type==='Annual'?'#DBEAFE':'#FEF9C3',color:r.filing_type==='Annual'?'#1E40AF':'#854D0E',fontWeight:700}}>{r.filing_type}</span></TD><TD>{fmt(eff(r))}</TD><TD><YN v={!r.documents_pending} t="Received" f="Pending"/></TD><TD><YN v={r.form_prepared}/></TD><TD><YN v={r.return_filed}/></TD><TD>{r.srn}</TD><TD><SBadge status={r.status}/></TD><TD>{r.ct_team_members?.display_name}</TD></>)} />
  </>)
}
function AuditTab({ clientId, fy }) {
  const [rows, setRows] = useState([]); const [load, setLoad] = useState(true)
  useEffect(() => { setLoad(true); supabase.from('audit_tracker').select('*,ct_team_members!audit_tracker_assigned_auditor_fkey(display_name)').eq('client_id',clientId).eq('fy_label',fy).then(({data})=>{setRows(data||[]);setLoad(false)}) },[clientId,fy])
  if(load) return <Spin />
  return <CTTable cols={['Audit Type','Due Date','Books','Working','UDIN','Signed','Filed','Status','Auditor']} rows={rows} empty={<Empty label="Audit"/>} render={r=>(<><TD bold>{r.audit_type}</TD><TD>{fmt(eff(r))}</TD><TD><YN v={r.books_received}/></TD><TD><YN v={r.audit_working_prepared}/></TD><TD>{r.udin_number||<YN v={false}/>}</TD><TD><YN v={r.audit_report_signed}/></TD><TD><YN v={r.filing_completed}/></TD><TD><SBadge status={r.status}/></TD><TD>{r.ct_team_members?.display_name}</TD></>)} />
}
function AccTab({ clientId, fy }) {
  const [rows, setRows] = useState([]); const [load, setLoad] = useState(true)
  useEffect(() => { setLoad(true); supabase.from('accounting_tracker').select('*,ct_team_members!accounting_tracker_assigned_to_fkey(display_name)').eq('client_id',clientId).eq('fy_label',fy).then(({data})=>{setRows(data||[]);setLoad(false)}) },[clientId,fy])
  if(load) return <Spin />
  return <CTTable cols={['Month','Sales','Purchase','Bank','GST Recon','TDS Recon','BRS','Closing','MIS','Status','Assigned']} rows={rows} empty={<Empty label="Accounting"/>} render={r=>(<><TD bold>{r.period_label||r.month}</TD><TD><YN v={r.sales_booked}/></TD><TD><YN v={r.purchase_booked}/></TD><TD><YN v={r.bank_entries_completed}/></TD><TD><YN v={r.gst_reconciliation_done}/></TD><TD><YN v={r.tds_reconciliation_done}/></TD><TD><YN v={r.bank_reconciliation_done}/></TD><TD><YN v={r.month_closing_done}/></TD><TD><YN v={r.mis_sent_to_client}/></TD><TD><SBadge status={r.status}/></TD><TD>{r.ct_team_members?.display_name}</TD></>)} />
}
function NoticeTab({ clientId }) {
  const [rows, setRows] = useState([]); const [load, setLoad] = useState(true)
  useEffect(() => { setLoad(true); supabase.from('notice_tracker').select('*,ct_team_members!notice_tracker_assigned_to_fkey(display_name)').eq('client_id',clientId).order('created_at',{ascending:false}).then(({data})=>{setRows(data||[]);setLoad(false)}) },[clientId])
  if(load) return <Spin />
  return <CTTable cols={['Authority','Type','Section','Notice Date','Response Due','Linked To','Reply Filed','Demand','Status','Assigned']} rows={rows} empty={<Empty label="Notices"/>} render={r=>(<><TD bold>{r.authority}</TD><TD>{r.notice_type}</TD><TD>{r.section}</TD><TD>{fmt(r.notice_date)}</TD><TD red={eff(r)&&new Date(eff(r))<new Date()}>{fmt(eff(r))}</TD><TD>{r.linked_compliance_period}</TD><TD><YN v={r.reply_filed}/></TD><TD>{r.demand_raised?'₹'+Number(r.demand_raised).toLocaleString('en-IN'):null}</TD><TD><SBadge status={r.status}/></TD><TD>{r.ct_team_members?.display_name}</TD></>)} />
}
...
function FirmDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('v_overdue_ageing').select('ageing_bucket, count:ageing_bucket'),
  ...
  const topCards = [
    {label:'Total Compliances', val:totals.total,     color:'#6366F1', icon:'📋'},
    ...
    {label:'Apprvl Pending',    val:totals.approval,  color:'#7C3AED', icon:'✍️'},
  ]

  const categoryIcons = { 'Income Tax':'🧾','GST':'🏪','TDS':'💰','ROC':'🏢','LLP':'🤝','Audit':'🔍','Accounting':'📒','Payroll':'👥','Trust/NGO':'🏛️','Notice':'📨' }
        ...
        <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:10, padding:'16px 20px' }}>
          ...
          {data.length===0 ? <Empty label="data"/> : data.map((row,i)=>(
            <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:i<data.length-1?'1px solid #F3F4F6':'' }}>
              <span style={{ fontSize:16 }}>{categoryIcons[row.category]||'📋'}</span>
    ...
    <div>
      ...
      <div className="card" style={{ overflow:'hidden' }}>
        {load ? <div style={{padding:32,textAlign:'center',color:'var(--gray2)'}}>Loading clients...</div>
          : filtered.length===0
          ? <div style={{padding:32,textAlign:'center',color:'var(--gray2)'}}>No active clients found.</div>
          : filtered.map(cl=>(
...
export default function Compliance({ user }) {
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:4, background:'#fff', border:'1px solid var(--border)', borderRadius:10, padding:4, marginBottom:20, width:'fit-content' }}>
        {mainTabs.map(t=>(
          <button key={t.id} onClick={()=>setMainTab(t.id)} style={{
            ...
            color:mainTab===t.id?'#fff':'var(--gray)',
            transition:'.15s'
      ...
      {/* Client-wise Tab */}
      {mainTab==='clients' && (
        <ClientComplianceList onSelect={cl=>setSelectedClient(cl)} />
      )}
