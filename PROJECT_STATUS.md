...
- **Governing HEAD (`ui/redesign-v1`):** `800013f6189a554db3e9107601430ee0aec2bb4b` (after PR #25 merge). **Last application/source-change commit:** `3a5f439c15cafa493cd2d2320d7733f286441b6b` (verified via `git log -- src`; every commit since is documentation-only).
...
- **Merged recovery design:** PR **#24** (complete historical recovery, consolidation & live-alignment design, Issue #23) → **merged `b2ceb30…`**.
- **Merged Package A discovery:** PR **#25** (Package A — historical evidence & live-state discovery, Issue #23) → **merged `800013f…`** (governing).
- **Active draft PR (proposed, NOT yet governing):** **#26** *(Package A — PJ Evidence Execution Pack, Issue #23)*. The status changes in this checkpoint block are **proposed inside the unmerged PR #26**; they become governing only after ChatGPT PASS + PJ-authorised merge.
...
- **Complete historical recovery programme:** **#23** (complete historical recovery, consolidation & live alignment) — active draft PR (design-only); branch `recovery/yav2-complete-history-live-alignment` from `798afaa`. See `docs/YAV2_Complete_Historical_Recovery_Consolidation_And_Live_Alignment_Plan.md`. Design goal: move the **complete approved YAV2 system** (architecture, DB/migrations/RLS/Auth/RBAC/security/privacy/storage/Edge/audit/automation/integrations/frontend/backend/config/runtime/docs/evidence) into one controlled V2 live state. **This first package is design + initial inventory only** — recovery design (architecture/DB/migration/RLS/Auth/security/functions/audit/frontend/Edge/storage/Vercel/n8n audit **plans**), an **initial** source/history inventory, historical decision register (initial inventory), approval-to-live reconciliation (provisional §18 states), recovery **Packages A–H**, security test matrix, closure standard. **Present status: COMPLETE RECOVERY DESIGN PREPARED · INITIAL SOURCE/HISTORY INVENTORY COMPLETED · FULL HISTORICAL AND LIVE RECONCILIATION PENDING EXECUTION OF PACKAGES A–H.** The historical **audit/reconciliation is NOT yet complete** — row-by-row decision register, object-by-object/per-table DB register, migration-by-migration V2 reconciliation, table-by-table RLS matrix, role-by-role Auth/RBAC evidence, feature-by-feature approval-to-live status, and live Vercel/Supabase/Edge/storage/n8n evidence are **Package A/B execution deliverables**. **Nothing implemented/executed/deployed**; every live action is a separate PJ gate. Live layers PENDING within standing guardrails (Claude never executes SQL; MCP reaches only prohibited V1; no test creds). *(Design PR #24 merged `b2ceb30…`.)*
  - **Package A (Issue #23) — active draft PR #25:** read-only historical evidence & live-state discovery. Deliverables `docs/YAV2_Package_A_Historical_Evidence_And_Live_State_Discovery.md` + `supabase/verification/YAV2_Package_A_V2_Live_State_Discovery_Readonly.sql` (PJ executes on V2). **Conclusion: `PACKAGE A PARTIAL — PJ EVIDENCE REQUIRED`.** Only the **Claude-side source/provenance discovery is complete** — baseline `b2ceb30` verified; Vercel deploy `dpl_6zvEGXX67…`@`b2ceb30` proves **deployment provenance only** (env values/scopes + Supabase target UNVERIFIED); full source/tables/RPCs/Edge/flags inventory; approval-to-live register, every item classified, no `UNKNOWN`. **Package A remains PARTIAL** until the **PJ evidence bundle is attached and reviewed**; it does not authorise Package B. **PJ evidence needed:** run the V2 discovery SQL (Part 1 catalog-safe + Part 2 optional probes; manual V2 confirmation, no auto-abort); supply Vercel env-var presence/scope/V2-target evidence (no secrets); confirm exact portal URL; approved test-account runtime evidence; n8n/WhatsApp export. Overall governance indicator ≈30% (provenance strong; config/live-DB/runtime unverified). *(Package A discovery PR #25 merged `800013f…`.)*
  - **Package A PJ Evidence Execution Pack (Issue #23) — active draft PR #26:** documentation-only step-by-step manual procedure (`docs/YAV2_Package_A_PJ_Evidence_Execution_Pack.md`) for PJ to collect the five outstanding evidence items — E1 V2 read-only SQL output (V2 `ogjrwemjefvccpyjwxuo` only; never V1; manual ref confirmation; Part 1 catalog-safe + Part 2 optional probes; probe errors recorded as findings, never "fixed"), E2 Vercel env presence/scope/V2-target (no secrets; reveal only the project-ref portion), E3 exact portal URL, E4 approved-test-account runtime evidence — **strictly read-only** (login/menus/navigation/direct-access/visibility/denial; **no writes** — all create/edit/delete/persistence marked `UNVERIFIED — WRITE TEST NOT AUTHORISED IN PACKAGE A`; no user creation/reset), E5 n8n/WhatsApp inventory (no execution/secret export). Part 1 is **designed to avoid** missing-object failures but may still error (permissions/timeout/session/platform) — such errors are **recorded verbatim, never "fixed" by DB/SQL change**. **Mandatory redaction check before any upload** (no tokens/keys/passwords/webhook secrets/secret-bearing URLs/unneeded client or personal data; keep only project ref/var name/role/status/error category). **Nothing executed/changed. Package A stays `PARTIAL`; Package B not authorised.**
...
_Last updated: 2026-07-23 IST (via Issue #23 Package A PJ Evidence Execution Pack — draft/unapproved)._
